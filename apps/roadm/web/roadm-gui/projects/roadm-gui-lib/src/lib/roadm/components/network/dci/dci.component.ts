...
export class DciComponent {
  ...
  if(this.recj){
    ...
    this.serviceElement.uuid=this.uuid_trans;
  }
  ...
  choose(i:number){
  this.del_uuid=this.serviceList[i].uuid;
  console.log(this.del_uuid);
  }
    SendDelete(){
                  if(this.wss.isConnected){
                      this.wss.sendEvent('dciDeleteRequest',{
                      'uuid':this.del_uuid,
                      });
                      console.log(this.del_uuid);
    ...
    //                 this.receiveData.time = data['creat_time'];
            ...
            this.handlers.push('dciDeleteResponse');
            this.SendDelete();
  ...
  console.log(aq);
  let bq = JSON.parse(this.uuid_rec);
  console.log(bq);
  this.uuid_trans=bq.data.uuid;
  this.recj = aq;
  this.sub();
  }
  ...
  ReceiveMessageFromBackward(){
        this.wss.bindHandlers(new Map<string,(data)=>void>([
            ['yiResponse',(data)=>{
                this.log.info(data);
                this.receive = data['DCI get message'];
                this.uuid_rec =data['DCI post message'];
            }]
        ]));
